...
class SnipeController(Controller):
    ...
    def RunBuyAndStoreClick(self):
        ...
        InputData= {
            ...
            'LongBreak': float(self.view.entries['LongBreak'].get())
        }
...
class DeepdiveMarketController(Controller):
    def __init__(self, model:CrocodileTrader) -> None:
        self.model = model
        self.view = None
    ...
    def bind(self, view = Form):
        self.view = view
        self.view.CreateDeepdiveMarketView()

        self.view.buttons['MarketScan'].configure(command = lambda: threading.Thread(target=self.MarketScan, daemon=True).start())
        self.view.buttons['BruteForceScan'].configue(command = lambda: threading.Thread(target=self.BruteForceScan, daemon=True).start())
    
    
    def MarketScan(self):
        print("New Thread is created for MarketScan")
    
    def BruteForceScan(self):
        print("New Thread is created for BruteForceScan")